...
module.exports = {
    index: async (ctx, next) => {
        ...
        if (ctx.bitrix.contact) {
            ...
            let users
            if (ctx.cnf.users) {
                users = await ctx.bitrix.bx.getUsers({
                    "sort": "ID",
                    "order": "ASC",
                    "FILTER": {
                        "ID": ctx.cnf.users,
            ...
            /*-----TEMP-----*/
            const logText = `Очередь товаров: OrderQueue orderID(${ctx.bitrix.lead.ID}) - ALL USER(${users.reduce((acc, u) => `${acc}, ${u.ID}`, '')}) | NEXT USER(${nextUserID})`;
